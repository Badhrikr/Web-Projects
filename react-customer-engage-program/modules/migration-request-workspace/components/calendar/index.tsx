...
import { GetCalendarForMigrationRequestResponse } from "../../services/model.response";
import { CalendarContainerProps } from "./model";

const localizer = momentLocalizer(moment);

function CalendarContainer({ migrationRequestId }: CalendarContainerProps) {
  ...
  const fetchSuccess = (response: GetCalendarForMigrationRequestResponse[]) => {
    setFetching(false);
  ...
  useEffect(() => {
    // "dd1519b0-401f-4932-b9d6-06558c146117"
    Services.GetCalendarForMigrationRequest({
      migrationRequestId,
