import { useRouter } from "next/router";
import { useEffect, useMemo, useRef, useState } from "react";
import { useInView } from "react-intersection-observer";
import { toast } from "react-toastify";
import { CSSTransition } from "react-transition-group";
import { filter, filter_funnel, search } from "../../../../../helpers/icons";
import useKeydown from "../../../../../hooks/use-keydown";
import useOutsideClick from "../../../../../hooks/use-outside-click";
import { NotificationObject } from "../../../../../shared/model";
import { ReceiveMessageforSpecificRole } from "../../../../../signalR/model";
import Connector from "../../../../../signalR/signalRconnection";
import Button from "../../../../../ui-components/Button";
import Icon from "../../../../../ui-components/Icon";
import IconButton from "../../../../../ui-components/IconButton";
import Input from "../../../../../ui-components/Input";
import Loading from "../../../../../ui-components/Loading";
import { CustomerUpdateType } from "../../../enums";
import { CustomersFilterProps } from "../../../model";
import * as Services from "../../../services";
import {
  CustomersResponse,
  GetAllCustomersResponse,
  PaginationParams,
} from "../../../services/model.response";
import FilterChip from "../../filter-chip";
import Customer from "../customer";
import { CustomerProps } from "../customer/model";
import CustomersApplyFilters from "../customers-apply-filters";
import { CustomersContainerProps } from "./model";

const LIMIT = 9;
const defaultFilters: CustomersFilterProps = {
  Offset: 0,
  Limit: LIMIT,
  SortField: null,
  SortOrder: null,
  Status: null,
  SearchText: null,
};

function CustomersContainer({ query }: CustomersContainerProps) {
  const router = useRouter();

  const validQueryParamFilters: CustomersFilterProps = useMemo(() => {
    let validFilters = {};
    Object.keys(query).forEach((filter) => {
      validFilters = {
        ...validFilters,
        ...(filter in defaultFilters && { [filter]: (query as any)[filter] }),
      };
    });

    return {
      ...defaultFilters,
      ...validFilters,
      ...(!("Offset" in query) && { Offset: 0 }),
      ...((!("Limit" in query) || Number(query["Limit"]) < 9) && {
        Limit: defaultFilters.Limit,
      }),
    };
  }, [query]);

  const [customers, setCustomers] = useState<Array<CustomerProps>>([]);
  const [filters, setFilters] = useState<CustomersFilterProps>(
    validQueryParamFilters
  );
  const [fetching, setFetching] = useState(true);
  const [filterText, setFilterText] = useState(
    validQueryParamFilters.SearchText ?? ""
  );
  const [showFilters, setShowFilters] = useState(false);
  const [pagination, setPagination] = useState<PaginationParams>();

  const { events, removeEvents } = Connector();

  const filtersAdded = useMemo(() => {
    return (
      Object.keys(filters)
        .filter(
          (filter) =>
            !["Offset", "Limit"].includes(filter as keyof typeof filters)
        )
        .filter((filter) => filters[filter as keyof typeof filters]).length > 0
    );
  }, [filters]);

  const [filtersContainerRef] = useOutsideClick(
    () => {
      setShowFilters(false);
    },
    { closeOnEsc: true }
  );
  useKeydown((e: KeyboardEvent) => {
    if (e.altKey && e.shiftKey && /^f$/i.test(e.key)) {
      setShowFilters((prev) => !prev);
    }
  });

  const containerRef = useRef(null);

  const { ref, inView } = useInView({
    threshold: 0.75,
  });

  const customersFetchSuccess = async ({
    data,
    pagination,
  }: GetAllCustomersResponse) => {
    setFetching(false);

    if (!data) return;

    setCustomers((customers) => [...customers, ...data]);
    setPagination(pagination);
  };

  const customersFetchError = () => {
    setFetching(false);
    toast("Error Occurred", {
      type: "error",
    });
  };

  const fetchCustomers = () => {
    setFetching(true);

    Services.GetAllCustomers({
      ...filters,
      success: customersFetchSuccess,
      error: customersFetchError,
    });
  };

  const toggleFilters = () => {
    setShowFilters((prev) => !prev);
  };

  const filterApplyHandler = (updatedFilters: CustomersFilterProps) => {
    setCustomers([]);
    setFilters({ ...updatedFilters, Limit: LIMIT, Offset: 0 });
    setShowFilters(false);
  };

  const filterResetHandler = () => {
    setCustomers([]);
    setFilters({ ...filters, ...defaultFilters });
    setShowFilters(false);
    setFilterText("");
  };

  const filterRemoveHandler = (filter: string) => {
    setCustomers([]);
    setFilters({
      ...filters,
      [filter]: null,
    });
  };

  const filterTextChangeHandler = ({ target: { value } }: any) => {
    setFilterText(value.trim());
  };

  const searchSubmitHandler = (e: React.FormEvent) => {
    e.preventDefault();
    if (filterText.length === 0) return;

    filterApplyHandler({ SearchText: filterText });
  };

  useEffect(() => {
    if (
      inView &&
      !fetching &&
      pagination &&
      customers.length < pagination?.totalCount
    ) {
      setFilters({
        ...filters,
        Offset: Number(filters.Offset ?? 0) + LIMIT,
      });
    }
  }, [inView]);

  useEffect(() => {
    fetchCustomers();

    // let queryParamsFilters = {};
    // Object.keys(filters).forEach((filter) => {
    //   if (filters[filter as keyof typeof filters] === null) return;

    //   queryParamsFilters = {
    //     ...queryParamsFilters,
    //     [filter]: filters[filter as keyof typeof filters],
    //   };
    // });

    // router.push({
    //   pathname: router.pathname,
    //   query: queryParamsFilters,
    // });
  }, [filters]);

  const receiveMessage = ({
    notification,
    data,
  }: ReceiveMessageforSpecificRole) => {
    const { action, detailId }: NotificationObject =
      JSON.parse(notification) ?? {};

    if (action === CustomerUpdateType.CustomerCreated) {
      const dataObject: CustomersResponse = JSON.parse(data);
      setCustomers((prevCustomers) => {
        return [
          {
            ...dataObject,
            Id: detailId,
            isNew: true,
          },
          ...prevCustomers,
        ];
      });
      return;
    }

    const isCustomerUpdate = Object.values(CustomerUpdateType).includes(
      action as CustomerUpdateType
    );

    if (isCustomerUpdate && detailId) {
      const dataObject: CustomersResponse = JSON.parse(data);

      setCustomers((prevCustomers) => {
        let index = prevCustomers.findIndex(
          (customer) => customer.Id === detailId
        );
        if (index !== -1) {
          prevCustomers[index] = dataObject;
        }
        return [...prevCustomers];
      });
    }
  };

  useEffect(() => {
    events(receiveMessage, receiveMessage);

    return () => {
      removeEvents(receiveMessage, receiveMessage);
    };
  }, []);

  return (
    <div className="flex flex-col gap-4" ref={containerRef}>
      <div className="flex items-center gap-2">
        <form onSubmit={searchSubmitHandler} className="flex-1 relative">
          <Input
            size="sm"
            theme="primary"
            icon={filter_funnel}
            placeholder="Filter by keyword"
            value={filterText}
            onChangeEvent={filterTextChangeHandler}
          />

          <div className="absolute right-1 top-1 bottom-0">
            <Button
              size="sm"
              theme="secondary"
              type="submit"
              className=" !bg-theme-background-popup cursor-pointer h-full px-3 flex-center-center"
            >
              <Icon icon={search} size="md" theme="secondary" />
            </Button>
          </div>
        </form>
        <div>
          <IconButton
            disabled={fetching}
            size="lg"
            theme="primary"
            className="!bg-[rgba(15,82,186,0.1)] !shadow-none toggle-button"
            onClick={toggleFilters}
          >
            <Icon
              icon={filter}
              size="md"
              theme="secondary"
              className="[&>svg]:!stroke-none"
            />
          </IconButton>
        </div>
      </div>

      <CSSTransition
        in={filtersAdded && !fetching}
        timeout={300}
        classNames="fly"
        unmountOnExit
      >
        <div className="flex items-center gap-4">
          <h3 className="text-sm">
            <span className="text-theme-secondary whitespace-nowrap">
              Records found:{" "}
            </span>
            <span className="text-theme-secondary font-semibold">
              {pagination?.totalCount ?? "?"}
            </span>
          </h3>

          <div className="flex flex-wrap gap-x-3 gap-y-3 items-center py-2">
            {Object.keys(filters).map(
              (key, i) =>
                !["Offset", "Limit"].includes(key) &&
                filters[key as keyof typeof filters] !== null && (
                  <FilterChip
                    key={i}
                    title={key}
                    filterText={filters[key as keyof typeof filters] as string}
                    onRemove={() => filterRemoveHandler(key)}
                  />
                )
            )}
          </div>
        </div>
      </CSSTransition>

      <CSSTransition
        in={showFilters}
        unmountOnExit
        timeout={300}
        classNames="slide-right-left"
      >
        <div
          ref={filtersContainerRef}
          className="flex fixed right-0 top-[7.30rem] bottom-0"
          // top-[3.75rem]
        >
          <CustomersApplyFilters
            {...filters}
            onApply={filterApplyHandler}
            onReset={filterResetHandler}
          />
        </div>
      </CSSTransition>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-6">
        {customers.map((customer, key) => (
          <Customer key={customer.Id + key} {...customer} />
        ))}
      </div>

      {!fetching && customers.length > 0 && <div ref={ref}></div>}

      {fetching && (
        <div className="py-5 flex gap-2 items-center justify-center">
          <Loading size="md" theme="primary" />
          <span className="text-theme-secondary">Loading</span>
        </div>
      )}

      {!fetching && customers.length === 0 && (
        <h3 className="text-center text-theme-secondary">No results found</h3>
      )}
    </div>
  );
}

export default CustomersContainer;
